...
export default function VenuesListingPage() {
  ...
  const handleInquirySubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        setFeedback(`Inquiry #${res.data.bookingId} created! Venue manager will contact you shortly.`);
  ...
  const filteredHalls = halls.filter(
    (h) =>
      ...
      (h.venue_type || '').toLowerCase() === selectedVenueType.toLowerCase() ||
      h.name.toLowerCase().includes(selectedVenueType.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F9] text-[#111827]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-[#F0D5E2] pb-6">
          <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-[#FAF5F7] border border-[#F0D5E2] text-xs font-bold text-[#22131A] uppercase"
                  >
